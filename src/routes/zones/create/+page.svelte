<script lang="ts">
	...
	let { data } = $props();
	let zone = $state(data.z || { n: '', l: 0, g: 0, i: '' });
</script>

<div class="mx-auto max-w-2xl p-4">
	<div class="mb-6">
		<a href="/zones" class="inline-flex items-center font-medium text-[var(--text-accent)] hover:text-[var(--accent-primary)]">&larr; Back to zones</a>
	</div>

	<h1 class="text-2xl font-bold text-[var(--accent-primary)] mb-6">Create Zone</h1>

	<form method="POST" use:enhance={{ onError: (e) => toast.error(e.detail.result.message || 'Failed to create zone') }}>
		...
		<div class="space-y-4">
			...
			<Button text="Create Zone" type="submit" wide={true} />
		</div>
	</form>
</div>