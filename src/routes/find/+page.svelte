<script lang="ts">
	...
	const search = async () => {
		...
		try {
			...
			console.error(
				'Failed to initialize or join meeting',
				e
			);
...
>
	<div class="max-w-2xl px-4 text-center">
		...
		>
			Meet Friends like you
		</h1>
		<p class="text-accent mb-8 text-xl">
			like Omegle. connect with people who share your
			interests. Filter by gender and age
		</p>
		...
		<div class="mt-8">
			...
			<button
				...
				class="btn-outline group mx-auto flex items-center gap-3 rounded-full px-6 py-3 font-bold shadow-lg transition-all hover:scale-105"
			>
				{searching
					? 'Stop searching'
					: 'Start searching'}
...
<style>
	.searching-text {
		...
		color: var(
			--text-accent
		); /* Assuming this is defined in app.css or similar */
	}
</style>
