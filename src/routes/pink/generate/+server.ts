...
export const POST: RequestHandler = async ({ request }) => {
	const { prompt, x, pid, m } = (await request.json()) as {
		prompt?: string;
		x?: string[];
		pid?: string;
		m?: boolean;
	...
	if (pid && m) {
		...
		if (!preset || preset.s !== 'p') throw error(404, 'preset not found');
		const need = preset.x?.length || 0;
		const have = Array.isArray(x) ? x.length : 0;
		if (need > 0 && need !== have) throw error(400, `upload exactly ${need} image(s)`);
	}

	if (!GEMINI) {
		return json({ images: [], text: (prompt || '').slice(0, 200) });
	}
	...
	async function fetch_base64(url: string): Promise<{ data: string; mimeType: string } | null> {
		try {
