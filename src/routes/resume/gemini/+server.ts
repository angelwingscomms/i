...
export const POST = async ({ request, locals }) => {
	...
	const resumes = await search_by_payload<Resume>({ s: 'e', t: 'resume', u: locals.user.i });
	if (resumes.length === 0) {
	...
	const id = resume.i;
	const updated = await edit_point(id, { h: new_h, l: Date.now() });
	return json({ success: true, resume: { ...resume, h: new_h, l: Date.now() } });
};