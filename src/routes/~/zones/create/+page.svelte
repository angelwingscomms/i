				...
				{#each place_results as place (place.place_id)}
					<li>
						...
						>
							...
							<p
								class="mt-1 text-xs text-[var(--color-theme-3)]"
							>
								lat {Number(place.lat).toFixed(5)} • lon
	...
	{#if map_visible || (zone.l && zone.g)}
		<div class="space-y-2">
			...
			<p
				class="text-xs text-[var(--color-theme-3)] lowercase"
			>
				move the pin or tap the map to set the exact
				spot
			</p>
		</div>
	{/if}
