...
export async function POST({ locals }) {
	...
	const [createdRooms, receivedRooms] =
		await Promise.all([
			search_by_payload<Pick<Room, 't' | 'l' | 'm'>>(
				{ s: 'r', u: userId },
				['t', 'l', 'm'],
				144
			),
	...
	const roomsById = new Map<
		string,
		{ i: string; t: string; l?: number; m?: number }
	>();
	...
	for (const r of membershipRooms)
		roomsById.set(r.i, r);
	...
	const result = Array.from(roomsById.values()).sort(
		(a, b) => (b.l || 0) - (a.l || 0)
	);
