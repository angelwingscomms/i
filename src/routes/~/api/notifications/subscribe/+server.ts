...
export const POST: RequestHandler = async ({
	...
	try {
		if (!locals.user) {
			notif_debug(
				'Subscribe POST: Unauthorized, no locals.user'
			);
			return json(
				{ error: 'Unauthorized' },
				{ status: 401 }
		...
		notif_debug(
			`Subscribe POST entry for user ${userId}`
		);
		const sub =
		...
		const now = Date.now();
		const subExp = (
			sub as unknown as { expirationTime?: number }
		).expirationTime;
		if (
			typeof subExp === 'number' &&
			subExp > 0 &&
			subExp < now
		) {
			notif_debug(
				`Sub expired for ${userId}: exp=${subExp}, now=${now}`
			);
			// Expired; acknowledge but don't store
			return json({ ok: true, pruned: true });
		}
		notif_debug(
			`Sub valid for ${userId}, no expiration issue`
		);
		...
		notif_debug(
			`Non-expired subs for ${user_id}: ${non_expired.length}/${list.length}`
		);
		const updated = [
			...non_expired.filter(
		...
		notif_debug(
			`Updated ps list for ${user_id}: length=${updated.length}`
		);
		...
		notif_debug(
			`Subscribe error for ${locals.user?.i || 'unknown'}: ${e instanceof Error ? e.message : String(e)}`
		);
		return json(
