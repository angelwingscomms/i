...
export const POST: RequestHandler = async ({
	...
	if (!(file instanceof File))
		throw error(400, 'file required');
	if (!file.type.startsWith('image/'))
		throw error(400, 'only image files allowed');
		...
		try {
			...
			const res = await model.generateContent({
				contents: [
					{
						role: 'user',
						parts: [{ text: prompt }, { text: url }]
					} // simple: provide URL reference
				]
			});
	...
	let room_id = '';
	try {
		const meeting = await realtime.post('meetings', {
			title: 'meme'
		});
