<script lang="ts">
  ...
  async function search() {
    ...
    try {
      ...
      console.error(e); errorMsg = 'Search failed';
...
<div class="container">
  ...
  <div class="search-bar">
    ...
    <input id="ytq" class="input" placeholder="search videos..." bind:value={query} onkeydown={(e) => e.key === 'Enter' && search()} />
  ...
  {#if showChat}
    <div class="modal_backdrop" role="button" tabindex="0" aria-label="Close chat" onclick={() => (showChat = false)} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (showChat = false)}></div>
...
<style>
  ...
  .title { font-size: 1.5rem; margin-bottom: 12px; }
  .search-bar { display: flex; gap: 8px; align-items: center; }
  .input { flex: 1; padding: 10px; border-radius: 8px; border: 1px solid var(--border-secondary); }
  .btn { padding: 10px 14px; border-radius: 8px; border: 1px solid var(--border-secondary); background: var(--bg-card); }
  .btn-secondary { padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border-secondary); background: transparent; }
  .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px; margin-top: 16px; }
  .card { border: 1px solid var(--border-secondary); border-radius: 10px; overflow: hidden; background: var(--bg-card); display:flex; flex-direction: column; }
  .thumb { width: 100%; aspect-ratio: 16 / 9; object-fit: cover; }
  .meta { padding: 10px; display:flex; flex-direction: column; gap: 8px; }
  .video-title { font-weight: 600; text-decoration: none; color: var(--text-primary); }
  .desc { color: var(--text-secondary); font-size: .9rem; line-height: 1.2rem; max-height: 4.8rem; overflow: hidden; }
  .stats { display:flex; gap: 12px; font-size: .8rem; color: var(--text-secondary); }
  .error { color: #b00; margin-top: 8px; }

  .modal_backdrop { position: fixed; inset: 0; background: rgba(0,0,0,.5); }
  .modal { position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); width: min(720px, 92vw); max-height: 92vh; background: var(--bg-primary); border: 1px solid var(--border-secondary); border-radius: 12px; display:flex; flex-direction: column; }
  .modal_header { display:flex; align-items: center; justify-content: space-between; padding: 10px 12px; border-bottom: 1px solid var(--border-secondary); }
  .modal_title { font-size: 1rem; font-weight: 600; }
  .modal_body { padding: 8px; }
  .close { background: transparent; border: none; font-size: 24px; line-height: 1; cursor: pointer; }
</style>
