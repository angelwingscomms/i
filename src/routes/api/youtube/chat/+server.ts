...
export const POST: RequestHandler = async ({ request }) => {
  const { title, description = '', transcript = '', messages = [] } = (await request.json()) as Body;
  if (!title?.trim()) throw error(400, 'title required');

  if (!GEMINI) {
    const last = messages.at(-1)?.text || '';
    return json({ t: `No API key configured. You asked: ${last.slice(0, 120)}...` });
  }
...
Keep answers concise. Include timestamps only if clear from text.`;
    ...
    const convo = [
      ...
      ...messages.map((m) => ({ role: m.role === 'user' ? ('user' as const) : ('model' as const), parts: [{ text: m.text }] }))
    ];

    const res = await model.generateContent({ contents: convo });
