...
export const POST: RequestHandler = async ({ request, locals }) => {
	try {
		if (!locals.user) {
			notif_debug('Unsubscribe POST: Unauthorized, no locals.user');
			return json({ error: 'Unauthorized' }, { status: 401 });
		}
		const userId = locals.user.i;
		notif_debug(`Unsubscribe POST entry for user ${userId}`);
		...
		notif_debug(`Updated ps list after remove for ${user_id}: length=${updated.length}`);
		await set(user_id, { ps: updated.length ? updated : null });
		notif_debug(`Set ps for ${user_id} after unsubscribe completed`);
		return json({ ok: true });
	} catch (e) {
		notif_debug(`Unsubscribe error for ${locals.user?.i || 'unknown'}: ${e instanceof Error ? e.message : String(e)}`);
