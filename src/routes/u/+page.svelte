<script lang="ts">
  ...
  async function search() {
    ...
    try {
      ...
      const arr = (await res.json()) as Result[];
      results = arr.sort((a, b) => (sort === 'match' ? (b.score ?? 0) - (a.score ?? 0) : (a.a ?? 0) - (b.a ?? 0)));
    } finally {
      loading = false;
    }
  }
  ...
  <div class="card-normal mb-6">
    <div class="flex flex-wrap items-center gap-3">
      <div class="choice-group">
        <label><input type="radio" class="sr-only" bind:group={gender} value={undefined} /><div class={gender==null?'choice-btn-active':'choice-btn-inactive'}>any</div></label>
        <label><input type="radio" class="sr-only" bind:group={gender} value={0} /><div class={gender===0?'choice-btn-active-blue':'choice-btn-inactive'}>male</div></label>
        <label><input type="radio" class="sr-only" bind:group={gender} value={1} /><div class={gender===1?'choice-btn-active':'choice-btn-inactive'}>female</div></label>
      </div>

      <div class="age-range-container ml-auto">
        <div class="flex items-center gap-2">
          <span class="text-sm text-[var(--text-tertiary)]">{minAge}</span>
          <input type="range" min="13" max="99" bind:value={minAge} on:input={() => { if (minAge > maxAge) maxAge = minAge; }} />
        </div>
        <div class="age-divider"></div>
        <div class="flex items-center gap-2">
          <input type="range" min="13" max="99" bind:value={maxAge} on:input={() => { if (maxAge < minAge) minAge = maxAge; }} />
      ...
      {#if data?.user}
        <div class="choice-group">
          <button class={mode==='profile'?'choice-btn-active':'choice-btn-inactive'} on:click={() => mode='profile'}>profile</button>
          <button class={mode==='custom'?'choice-btn-active':'choice-btn-inactive'} on:click={() => mode='custom'}>custom</button>
        </div>
      {/if}
      ...
      <div class="ml-auto flex items-center gap-3">
        ...
        <button class="btn-primary btn-md" on:click={search} disabled={loading}>{loading?'searching…':'search'}</button>
      </div>
    </div>

    {#if !data?.user || mode==='custom'}
      <div class="mt-3">
        <input class="input-rect w-full" placeholder="add a short description to match on (optional)" bind:value={description} />
      </div>
    {/if}
  </div>
      ...
      <a class="no-underline" href={`/u/${u.i}`}>
        <div class="result-card">
          <div class="flex items-center gap-3">
            ...
            <div class={`rounded-full px-2 py-1 text-xs font-semibold text-white ${badgeColor(p)}`}>{p===null?'—':`${p}%`}</div>
          </div>
        </div>
      </a>
    {/each}
