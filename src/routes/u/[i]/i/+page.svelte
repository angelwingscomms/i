	...
	<div class="items-grid mx-auto max-w-6xl px-4 pb-16 opacity-0">
				...
				{#each data.items as item (item.i)}
					<a href="/i/{item.i}" class="item-card card-hover card-normal no-underline">
						<div class="mb-4">
							<div class="mb-2 flex items-center justify-between">
								<span
									...
									style="border: 1px solid var(--color-theme-{item.k === 0
										? '1'
										: '2'}); color: var(--color-theme-{item.k === 0 ? '1' : '2'});"
								>
							...
							<h3 class="mb-2 text-xl font-bold" style="color: var(--color-theme-4);">
								{item.t}
							</h3>
							{#if item.d}
						...
						{#if item.x && item.x.length > 0}
							<div class="mb-4">
								<img src={item.x[0]} alt={item.t} class="h-32 w-full rounded-lg object-cover" />
							</div>
						{/if}
		...
		{:else}
			<div class="py-16 text-center">
				...
				<p class="mb-8 text-lg" style="color: var(--color-theme-6);">
					Be the first to share something amazing!
				</p>
				{#if data.user}
					<a href="/i/create" class="btn-primary btn-lg"> Create First Item </a>
				{:else}
					<a href="/google" class="btn-primary btn-lg"> Sign In to Create </a>
				{/if}
			</div>
		{/if}
	</div>
