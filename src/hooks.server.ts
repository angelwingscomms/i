...
type MinimalSession = { u: string; c: number; l: number; h: string; s: 'se' };

export const handle: Handle = async ({ event, resolve }) => {
  const sessionToken = event.cookies.get(sessionCookieName);
  const sessionJWT = event.cookies.get(sessionJwtCookieName);
  ...
  if (sessionJWT) {
    ...
    if (vs) {
      ...
      const msRemaining = vs.expiresAt.getTime() - Date.now();
      if (msRemaining <= 5 * 60 * 1000) {
        try {
          const newJwt = await createSessionJWT({ id: vs.id, createdAt: vs.createdAt });
          setSessionJwtCookie(event, newJwt);
        } catch (e) {
          console.debug('createSessionJWT error (rolling)', e);
        }
      } else {
        // Refresh cookie with the existing token to keep cookie alive until token expiry
        setSessionJwtCookie(event, sessionJWT);
      }
      if (sessionToken) setSessionTokenCookie(event, sessionToken);
