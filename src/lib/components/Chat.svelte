<script lang="ts">
	...
	import type { SendChatMessage, ChatMessage, Room } from '$lib/types';
	...
	$effect(() => {
		if (t.length > ROOM_NAME_DISPLAY_LIMIT) {
			display_room_name = t.slice(0, ROOM_NAME_DISPLAY_LIMIT) + '...';
		} else {
			display_room_name = t;
		}
	});
	...
	>
		{#if meeting?.chat.messages}
			{#each chat_messages as msg, i (msg.i)}
				<a
					class="chat_item {msg.u === page.data.user?.i ? 'justify-end' : 'justify-start'} items-end"
				...
				>
					<div
						class="chat_meta {msg.x === page.data.user?.t ? 'justify-end' : 'justify-start'}"
					>
					...
					<div
						class={`chat_bubble ${msg.x === page.data.user?.t ? 'chat_bubble--self' : ''} ${msg.x ? '' : 'chat_bubble--anon'} ${msg.saved ? '' : 'chat_bubble--pending'} ml-auto mr-auto max-w-[90%]`}
					>
