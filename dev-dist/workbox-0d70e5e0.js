define(['exports'], function (exports) {
    ...
    */
	const logger = (() => {
		...
		const api = {};
		const loggerMethods = Object.keys(
			methodToColorMap
		);
    ...
    */
	...
	class Router {
		...
		handleRequest({ request, event }) {
			...
			if (
				!handler &&
				this._defaultHandlerMap.has(method)
			) {
				{
					debugMessages.push(
	...
	function registerRoute(capture, handler, method) {
		...
		if (typeof capture === 'string') {
			...
			);
			{
				...
				const valueToCheck = capture.startsWith(
					'http'
				)
					? captureUrl.pathname
					: capture;
    ...
    */
	...
	function printInstallDetails(
		...
		const alreadyPrecachedCount =
			urlsAlreadyPrecached.length;
    ...
    */
	...
	async function cacheMatchIgnoreParams(
		...
		for (const cacheKey of cacheKeys) {
			...
			if (
				strippedRequestURL === strippedCacheKeyURL
			) {
	...
	class StrategyHandler {
		...
		async cachePut(key, response) {
			...
			const responseToCache =
				await this._ensureResponseSafeToCache(
					response
				);
			if (!responseToCache) {
				{
		...
		*iterateCallbacks(name) {
			for (const plugin of this._strategy.plugins) {
				if (typeof plugin[name] === 'function') {
					const state =
						this._pluginStateMap.get(plugin);
		...
		async doneWaiting() {
			...
			while (
				(promise =
					this._extendLifetimePromises.shift())
			) {
				await promise;
			}
		}
    ...
    */
	...
	class PrecacheStrategy extends Strategy {
		 ...
		 */
		constructor(options = {}) {
			options.cacheName = cacheNames.getPrecacheName(
				options.cacheName
			);
			super(options);
		...
		async _handle(request, handler) {
			...
			if (
				handler.event &&
				handler.event.type === 'install'
			) {
				return await this._handleInstall(
					request,
					handler
				);
			}
			...
			return await this._handleFetch(
				request,
				handler
			);
		}
	...
	class PrecacheController {
		...
		constructor({
			...
			this._strategy = new PrecacheStrategy({
				cacheName:
					cacheNames.getPrecacheName(cacheName),
				plugins: [
					...plugins,
		...
		addToCacheList(entries) {
			...
			for (const entry of entries) {
				...
				if (
					...
					if (
						this._cacheKeysToIntegrities.has(
							cacheKey
						) &&
						this._cacheKeysToIntegrities.get(
							cacheKey
						) !== entry.integrity
					) {
						throw new WorkboxError(
		...
		install(event) {
			...
			return waitUntil(event, async () => {
				const installReportPlugin =
					new PrecacheInstallReportPlugin();
				this.strategy.plugins.push(
					installReportPlugin
				);
				// Cache entries one at a time.
				// See https://github.com/GoogleChrome/workbox/issues/2528
				for (const [url, cacheKey] of this
					._urlsToCacheKeys) {
					const integrity =
						this._cacheKeysToIntegrities.get(
							cacheKey
					...
					const cacheMode =
						this._urlsToCacheModes.get(url);
				...
				const { updatedURLs, notUpdatedURLs } =
					installReportPlugin;
				{
					printInstallDetails(
						updatedURLs,
		...
		getCacheKeyForURL(url) {
			const urlObject = new URL(url, location.href);
			return this._urlsToCacheKeys.get(
				urlObject.href
			);
		}
		/**
		...
		getIntegrityForCacheKey(cacheKey) {
			return this._cacheKeysToIntegrities.get(
				cacheKey
			);
		}
		/**
		...
		constructor(precacheController, options) {
			const match = ({ request }) => {
				...
				for (const possibleURL of generateURLVariations(
					...
					const cacheKey =
						urlsToCacheKeys.get(possibleURL);
					if (cacheKey) {
						const integrity =
    ...
    */
	...
	function addRoute(options) {
		...
		const precacheRoute = new PrecacheRoute(
			precacheController,
			options
		);
    ...
    */
	...
	function cleanupOutdatedCaches() {
		...
		self.addEventListener('activate', (event) => {
			...
			event.waitUntil(
				deleteOutdatedCaches(cacheName).then(
					(cachesDeleted) => {
						{
    ...
    */
	...
	class NavigationRoute extends Route {
		...
		constructor(
			...
			{
				finalAssertExports.isArrayOfClass(
					allowlist,
					RegExp,
					{
						moduleName: 'workbox-routing',
    ...
    */
	...
	function createHandlerBoundToURL(url) {
		const precacheController =
			getOrCreatePrecacheController();
		return precacheController.createHandlerBoundToURL(
			url
		);
	}

	exports.NavigationRoute = NavigationRoute;
	exports.cleanupOutdatedCaches =
		cleanupOutdatedCaches;
	exports.clientsClaim = clientsClaim;
	exports.createHandlerBoundToURL =
