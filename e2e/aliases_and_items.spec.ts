...
test.describe('Nested aliases and item create', () => {
  ...
  test.beforeAll(async () => {
    userId = await create({ s: 'u', t: 'alias-user', d: 'desc', a: 21, g: 0, c: {} }, 'alias-user');
    roomId = await create({ s: 'r', t: 'alias-room', d: 'room desc', u: userId, c: 'cfid' }, 'alias-room');
  });
  ...
  test('user alias /u/t/:t redirects to canonical /u/:i', async ({ page }) => {
    await page.goto(`/u/t/alias-user`);
    await expect(page).toHaveURL(new RegExp(`/u/${userId}$`));
  });

  test('room alias /r/t/:t redirects to canonical /r/:i', async ({ page }) => {
    await page.goto(`/r/t/alias-room`);
    await expect(page).toHaveURL(new RegExp(`/r/${roomId}$`));
  });

  test('create item flow (minimal)', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/i\/create|\/google/);
  });
});
