...
test.describe('PWA', () => {
  test('has manifest link in head and loads manifest', async ({ page, context }) => {
    const respPromise = page.waitForResponse((r) => r.url().endsWith('/manifest.webmanifest'));
    await page.goto('/');
    // Check manifest link tag present
    const hasManifestLink = await page.locator('head link[rel="manifest"]').count();
  ...
  test('registers a service worker', async ({ page }) => {
    ...
    const swController = await page.evaluate(async () => {
      // @ts-ignore
    ...
    expect(swController.active || swController.waiting || swController.installing).toBeTruthy();
  });

  test('serves offline fallback for navigations when network is cut', async ({ page, context }) => {
    await page.goto('/');
    ...
    const resp = await page.goto('/some-unknown-offline-route-' + Date.now(), { waitUntil: 'domcontentloaded' });
  ...
  test('caches static assets (icons) via service worker', async ({ page, context }) => {
    await page.goto('/');
    ...
    await context.setOffline(true);
    const loadedOffline = await page.evaluate(async (src) => {
      const img = new Image();
      img.src = src + '?t=' + Date.now();
      try {
        await new Promise((res, rej) => {
          img.onload = () => res(true);
          img.onerror = () => rej(new Error('error'));
        });
        return true;
      } catch {
        return false;
