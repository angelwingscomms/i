...
test.describe('Room Creation', () => {
	test('should allow a user to create a room and redirect to the room page', async ({
		page
	}) => {
		...
		await page.fill(
			...
			'This is a description for my test room.'
		);
		...
		await expect(
			page.locator('.toast-success')
		).toHaveText('Room created successfully!');
	...
	test('should display validation errors for empty fields', async ({
		...
		await expect(
			page.locator('#room_tag_error')
		).toHaveText('Room Tag is required.');
		await expect(
			page.locator('#description_error')
