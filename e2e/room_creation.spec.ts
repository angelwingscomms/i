...
test.describe('Room Creation', () => {
    ...
    test('should display validation errors for exceeding max length', async ({ page }) => {
        ...
        await expect(page.locator('#room_tag_error')).toHaveText('Room Tag must be 50 characters or less.');
        await expect(page.locator('#description_error')).toHaveText('Description must be 200 characters or less.');
        await expect(page.locator('.toast-error')).toBeVisible();
    });
});
